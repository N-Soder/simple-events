...
attribution required, modification allowed. Each was cropped, graded and
re-encoded by `scripts/build-banner-presets.mjs`, so none is an unaltered copy.
...
| `picnic` | TODO: confirm. The hamper-on-a-gingham-blanket shot, not the park crowd. |
| `confetti` | https://www.pexels.com/photo/colorful-confetti-against-white-background-25956373/ |

Photographer names are worth filling in from each photo page when someone is next
in here. Pexels does not require it, but a name is more use than a URL if a photo
ever has to be traced.
